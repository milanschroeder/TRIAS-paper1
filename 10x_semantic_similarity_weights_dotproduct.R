...
# Adapted from: https://gist.github.com/tjvananne/8b0e7df7dcad414e8e6d5bf3947439a9
# N.B.: Expects vectors with tokens in columns and dimensions in rows 

find_sim_wvs <- function(seed_dict, concept,  embeddings = read_rds("./large_data/glove.6B.300d.rds")){
...
# coop-conflict
seed_coop <- c("cooperation", "agreement",    "support",    "collaboration", "unity")
seed_conf <- c("conflict",    "disagreement", "opposition", "confrontation", "hostility")
...
# Extract dot_product similarities to this average vector
# (word weights that would semantically pull a text towards the seed)
dp_simils <-  find_sim_wvs(seed_dp_simple, "DP_radical") 

# Export token weights
# Export semantic similarity dictionary
write_rds(dp_simils, "./large_data/weights_dot/SemSimilWeights-DigitalitySimple.rds", compress = "gz")


# Visualize 
pl <-
  ggplot(dp_simils %>% ungroup() %>% arrange(desc(concept)) %>% head(200), 
...
ggsave("./output/plots/SemanticallySimilarTerms_Digital_Simple_dot.png", pl, width = 24, height = 12, units = "cm")
...
dp_simils2 <- find_sim_wvs(seed_dp_adv, "DP_advanced") 


# Export token weights
# Export semantic similarity dictionary
write_rds(dp_simils2, "./large_data/weights_dot/SemSimilWeights-DigitalityAdvanced.rds", compress = "gz")

# Visualize 
pl <-
  ggplot(dp_simils2 %>% ungroup() %>% arrange(desc(concept)) %>% head(200), 
         aes(label = token, size = concept, color = seed)) +
  geom_text_wordcloud() +
  scale_radius(range = c(1.5, 6), limits = c(0, NA)) +
  scale_colour_manual(values = c("blue", "red"))+
  ...
  theme_minimal()+
  theme(plot.background = element_rect(color = "white"))
ggsave("./output/plots/SemanticallySimilarTerms_Digital_Advanced_dot.png", pl, width = 24, height = 12, units = "cm")
...
dp_simils3 <- dp_simils2 %>% 
  ...
  #        weight = sim.norm - perc.norm) %>% 
  mutate(sim.norm = (concept - min(concept, na.rm = T))/(max(concept, na.rm = T) - min(concept, na.rm = T)),
         perc.norm = (ngram.perc - min(ngram.perc, na.rm = T))/(max(ngram.perc, na.rm = T) - min(ngram.perc, na.rm = T)),
...
ggplot(dp_simils3, aes(x = concept, y = weight)) + 
  geom_point()
...
dp_simils3 <- dp_simils3 %>% 
  ...
  rename(concept = weight)

write_rds(dp_simils3, "./large_data/weights_dot/SemSimilWeights-DigitalityAdvancedFreqCorrection.rds", compress = "gz")

# Visualize 
pl <-
  ggplot(dp_simils3 %>% ungroup() %>% arrange(desc(concept)) %>% head(200), 
         aes(label = token, size = concept, color = seed)) +
  geom_text_wordcloud() +
  ...
  theme_minimal()+
  theme(plot.background = element_rect(color = "white"))
ggsave("./output/plots/SemanticallySimilarTerms_Digital_Advanced_FreqCorrection_dot.png", pl, width = 24, height = 12, units = "cm")
...
econ_simils <-  find_sim_wvs(seed_econ, "economics")

# Export token weights
# Export semantic similarity dictionary
write_rds(econ_simils, "./large_data/weights_dot/SemSimilWeights-Economy.rds", compress = "gz")


# Visualize 
pl <-
  ggplot(econ_simils %>% ungroup() %>% arrange(desc(concept)) %>% head(200), 
         aes(label = token, size = concept, color = seed)) +
  geom_text_wordcloud() +
  scale_radius(range = c(1.5, 6), limits = c(0, NA)) +
  scale_colour_manual(values = c("blue", "red"))+
  ...
  theme_minimal()+
  theme(plot.background = element_rect(color = "white"))
ggsave("./output/plots/SemanticallySimilarTerms_Economy_dot.png", pl, width = 24, height = 12, units = "cm")
...
sec_simils <- find_sim_wvs(seed_security, "security") 

# Export token weights
# Export semantic similarity dictionary
write_rds(sec_simils, "./large_data/weights_dot/SemSimilWeights-Security.rds", compress = "gz")


# Visualize 
pl <-
  ggplot(sec_simils %>% ungroup() %>% arrange(desc(concept)) %>% head(200), 
         aes(label = token, size = concept, color = seed)) +
  geom_text_wordcloud() +
  scale_radius(range = c(1.5, 6), limits = c(0, NA)) +
  scale_colour_manual(values = c("blue", "red"))+
  ...
  theme_minimal()+
  theme(plot.background = element_rect(color = "white"))
ggsave("./output/plots/SemanticallySimilarTerms_Security_dot.png", pl, width = 24, height = 12, units = "cm")
...
# (word weights that would semantically pull a text towards the seed)
lib_simils <-  find_sim_wvs(seed_librigths, "liberal_rights")

# Export token weights
write_rds(lib_simils, "./large_data/weights_dot/SemSimilWeights-LibRights.rds", compress = "gz")


# Visualize 
pl <-
  ggplot(lib_simils %>% ungroup() %>% arrange(desc(concept)) %>% head(200), 
         aes(label = token, size = concept, color = seed)) +
  geom_text_wordcloud() +
  scale_radius(range = c(1.5, 6), limits = c(0, NA)) +
  scale_colour_manual(values = c("blue", "red"))+
  ...
  theme_minimal()+
  theme(plot.background = element_rect(color = "white"))
ggsave("./output/plots/SemanticallySimilarTerms_Librights_dot.png", pl, width = 24, height = 12, units = "cm")



# Semantic scaling COOPERATION/CONFLICT ####

# Extract dot product similarities to this average vector
# (word weights that would semantically pull a text towards the seed)
wvs_coop_conf <- 
  find_similar_wvs(seed_dict = seed_coop, concept = "coop") %>% 
  select(token, coop = concept) %>% 
  left_join(.,
            find_similar_wvs(seed_dict = seed_conf, concept = "conf") %>% 
              select(token, conf = concept),
            by = "token") %>% 
  mutate(coop_conf = coop - conf,
         seed_coop = token %in% seed_coop,
         seed_conf = token %in% seed_conf) %>% 
  arrange(desc(coop_conf))

# Export token weights
write_rds(wvs_coop_conf %>% select(token, coop_conf), "./large_data/weights_dot/SemSimilWeights-ConflictCooperation.rds", compress = "gz")


# Visualize (ind scales)

pl <-
  ggplot(wvs_coop_conf %>% ungroup() %>% arrange(desc(coop)) %>% head(200), 
         aes(label = token, size = coop, color = seed_coop)) +
  geom_text_wordcloud() +
  scale_radius(range = c(1.5, 6), limits = c(0, NA)) +
  scale_colour_manual(values = c("blue", "red"))+
  ...
  theme_minimal()+
  theme(plot.background = element_rect(color = "white"))
ggsave("./output/plots/SemanticallySimilarTerms_Cooperation_dot.png", pl, width = 24, height = 12, units = "cm")

pl <-
  ggplot(wvs_coop_conf %>% ungroup() %>% arrange(desc(conf)) %>% head(200), 
         aes(label = token, size = conf, color = seed_conf)) +
  geom_text_wordcloud() +
  scale_radius(range = c(1.5, 6), limits = c(0, NA)) +
  scale_colour_manual(values = c("blue", "red"))+
  ...
  theme_minimal()+
  theme(plot.background = element_rect(color = "white"))
ggsave("./output/plots/SemanticallySimilarTerms_Conflict_dot.png", pl, width = 24, height = 12, units = "cm")
...
# Stratified sample
df <- wvs_coop_confl %>% 
  mutate(group = cut(coop_conf, 5)) %>% # Cut range of scale into n intervals
  group_by(group) %>% 
 # sample_n(size = 17) %>% # n words from each interval
  ungroup()
...
df.anchors <- wvs_coop_conf %>% 
  select(-starts_with("seed")) %>% 
  filter(token %in% c(seed_coop, seed_conf)) %>% 
  mutate(group = "Anchor terms defined by researcher")

df.learned <- wvs_coop_conf %>% 
  select(-starts_with("seed")) %>% 
...
pl<- 
  ...
  geom_text(data = df, alpha= .4, color = "grey60", aes(y=sample.int(100, size = nrow(df), replace =T), x = coop_conf, label = token)) +
  geom_vline(xintercept = 0, linetype = "dashed")+
  geom_text(data = df.highlight, aes(y=sample.int(100, size = nrow(df.highlight), replace =T), x = coop_conf, label = token, color = group), fontface = "bold")+
...
ggsave("./output/plots/ConflictCoopScalingWeigths_dot.png", pl, width = 28, height = 14, units = "cm")
