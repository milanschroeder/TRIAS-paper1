...
#             join_by(doc_key)
#   ) %>% 
#   select(-c(BI:WS), all_of(countrycode::codelist %>% filter(eu28 == "EU") %>% pull(iso2c))) %>% 
#   pivot_longer(all_of(
...
#   select(-eu)
# 
# # takes surprisingly long time, so save for future:
# write_rds(EU_mentions, "./data/EU_mentions")
# rm(EU_at_time, EU_mentions)
# gc()
